...
function AddApplicant() {
  ...
  const validate = () => {
    ...
    if (!formData.collegeName) {
      ...
      errors.collegeName =
        "College name should only contain alphabets and spaces";
    }
    ...
    if (!formData.resumeLink) {
      ...
      !/^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/i.test(formData.resumeLink.trim())
    ) {
      errors.resumeLink = "Resume link should be a valid URL";
    }
    if (formData.isExperienced === "") {
      errors.isExperienced =
        "Please confirm whether the applicant has experience or not";
    }
    if (formData.isExperienced === "yes") {
      if (!formData.previousCompany) {
        ...
        errors.previousCompany =
          "Previous company should only contain alphabets and spaces";
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    let toastShown = false; // Flag to control toast notifications
    if (validate()) {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_API_URL}/applicant/add`,
          formData
        );
        if (response.status === 201) {
          toast.info(`New Applicant ${formData.name} Added Successfully`, {
            autoClose: 2000,
            style: { fontSize: "16px" },
          });
          ...
          if (!toastShown) {
            ...
            toast.error(
              response.data?.email ||
                "Unable to add applicant now! Try after some time.",
              { autoClose: 3000, style: { fontSize: "16px" } }
            );
            toastShown = true; // Set flag to true
          }
        }
          ...
          >
            ...
            <MDBox mb={2}>
              <FormControl fullWidth>
                <InputLabel id="role-select-label">Applied Role</InputLabel>
                <Select
            ...
            <MDBox mb={2}>
              <FormControl fullWidth>
                <InputLabel id="qualification-select-label">
            ...
            <MDBox mb={2}>
              <FormControl fullWidth>
                <InputLabel id="isExperienced-select-label">
                  Experience Status
                </InputLabel>
                <Select
                  labelId="isExperienced-select-label"
                  name="isExperienced"
            ...
            <MDBox mb={2}>
              <FormControl fullWidth>
                <InputLabel id="sourceOfProfile-select-label">
                  Source of Profile
                </InputLabel>
                <Select
                  labelId="sourceOfProfile-select-label"
                  name="sourceOfProfile"
                  value={formData.sourceOfProfile}
                  onChange={handleChange}
                  style={selectHeight}
                  label="Source of Profile"
                >
                  <MenuItem value="" disabled>
                    ---Select Source of Profile---
                  </MenuItem>
                  {sourceOfProfileList.map((a) => (
            ...
            {(formData.sourceOfProfile === "Consultancy" ||
              formData.sourceOfProfile === "Institute") && (
              <MDBox mb={2}>
                <MDInput
                  type="text"
                  label={`Name of ${formData.sourceOfProfile}`}
                  fullWidth
                  name="sourceName"
